...
formal verification, and generation and execution of test cases.
...
In this step, for each user action that is relevant for the plug-in, 
...
and the system's outgoing responses. Incoming external events are
actions that determine the system to
change its state while outgoing internal events are the
responses the system sends as a reaction to incoming external events.
\autoref{example:event_chart_in} illustrates an incoming
event named
\emph{TYPECHECKALL}, triggered by a user action. The system response
to
this action is captured in an \emph{outgoing event} in
...
its architecture. Therefore the high
level classifiers described in
the system domain model  are transformed into concrete
data types. By applying the transformation, the \emph{system\_chart} presented in
...
This step captures the components interaction and
the interfaces the components present to each other. The events identified in Step 3 help
...
evaluating the scenarios from Step 2. \autoref{example:event_chart_in} 
...
\note{1) Mention the fact that the method has been used for 20 years on dozens of projects

2) Questions from the reviewer 1:
Are these steps performed in sequence (waterfall model) or in some iterative manner (e.g., spiral)?
Also, I do not understand what about this methodology is plug-in development specific? 
The authors should make it clearer in the paper 

3) Reference eRAISE master thesis which is in the bib file ~\cite{Fasie13}}
